...
class _af_design:
  def log_wandb(self, wandb_run=None, design_mode=None, lr_scale=None):
      ...
      log_dict = {k: log[k] for k in keys if k in log}
      log_dict['design_mode'] = design_mode
      if 'af_grad' in log:
          log_dict['af_grad_norm'] = np.linalg.norm(log['af_grad'])
      if 'ablm_grad' in log:
          log_dict['ablm_grad_norm'] = np.linalg.norm(log['ablm_grad'])
      if 'total_grad' in log:
          log_dict['total_grad_norm'] = np.linalg.norm(log['total_grad'])
      if 'ablm_ll' in log:
          log_dict['ablm_ll'] = log['ablm_ll']
      log_dict['lr_scale'] = lr_scale
      if '_ablm_scale' in self.opt:
          log_dict['ablm_scale'] = self.opt['_ablm_scale']
  ...
  def step(self, lr_scale=1.0, num_recycles=None, num_models=None, sample_models=None,
           ...
           save_filters=None, seq_entropy_threshold=None, iter=None, wandb_run=None):
  ...
  def design(self, iters=100,
             ...
             design_mode=None, save_filters=None, seq_entropy_threshold=None, wandb_run=None):
    ...
    for i in range(iters):
      ...
      self.log_wandb(wandb_run=wandb_run, design_mode=design_mode, lr_scale=lr_scale)

  def design_logits(self, iters=100, wandb_run=None, **kwargs):
    ''' optimize logits '''
    self.design(iters, design_mode='logits', wandb_run=wandb_run, **kwargs)

  def design_soft(self, iters=100, temp=1, wandb_run=None, **kwargs):
    ''' optimize softmax(logits/temp)'''
    self.design(iters, soft=1, temp=temp, design_mode='soft', wandb_run=wandb_run, **kwargs)

  def design_hard(self, iters=100, wandb_run=None, **kwargs):
    ''' optimize argmax(logits) '''
    self.design(iters, soft=1, hard=1, design_mode='hard', wandb_run=wandb_run, **kwargs)
  ...
  def design_semigreedy(self, iters=100, tries=10, dropout=False, save_filters=None,
        save_best=True, seq_logits=None, e_tries=None, get_best=False, wandb_run=None, **kwargs):
    ...
    for i in range(iters):
      ...
      for t in range(int(num_tries)):
        ...
        self.log_wandb(wandb_run=wandb_run, design_mode='semigreedy', lr_scale=None)
  ...
  def design_pssm_semigreedy(self, soft_iters=300, hard_iters=32, tries=10, e_tries=None,
                             ramp_recycles=True, ramp_models=True, get_best=False, wandb_run=None, **kwargs):
    ...
    if hard_iters > 0:
      ...
      if ramp_models:
        ...
        for m in range(num_models):
          ...
          self.design_semigreedy(iters, seq_logits=seq_logits, tries=tries, e_tries=e_tries, get_best=get_best, wandb_run=wandb_run, **kwargs)
          if m < 2: iters = iters // 2
      else:
        self.design_semigreedy(hard_iters, seq_logits=seq_logits, tries=tries, e_tries=e_tries, get_best=get_best, wandb_run=wandb_run, **kwargs)
